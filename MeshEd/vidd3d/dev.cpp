...
U32 VidD3D::create_device(XWnd *wnd,U32 width,U32 height,U32 byte_pp)
{
	...
	D3DDISPLAYMODE mode;
	HRESULT hResult;
	U32 has_stencil=FALSE;

	hResult = id3d->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &mode);
	if (hResult!=D3D_OK) {
		DXTrace(__FILE__,__LINE__, hResult, "Couldn't Get Adapter Display Mode", TRUE);
		return FALSE;
	}
	...
	/*if (id3d->CreateDevice(adaptor_id,D3DDEVTYPE_HAL,wnd->get_hwnd(),D3DCREATE_MIXED_VERTEXPROCESSING,&present,&dev)!=D3D_OK)
		return FALSE;
	*/
	hResult = id3d->CreateDevice(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,wnd->get_hwnd(),D3DCREATE_HARDWARE_VERTEXPROCESSING,&present,&dev);
	if (hResult!=D3D_OK) {
		DXTrace(__FILE__,__LINE__, hResult, "Couldn't Create D3D8 Device", TRUE);
		return FALSE;
	}
	if (id3d->GetDeviceCaps(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,&caps)!=D3D_OK)
